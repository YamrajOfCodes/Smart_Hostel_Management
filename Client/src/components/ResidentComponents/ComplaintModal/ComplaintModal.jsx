...
const FieldLabel = ({ children }) => (
  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{children}</p>
);
...
const CATS = ["Plumbing","Electrical","WiFi/IT","Carpentry","Housekeeping","Mess/Food","Laundry","Other"];
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {[
              { k:"low",    active:"bg-emerald-500 text-white border-emerald-500" },
              { k:"medium", active:"bg-amber-500   text-white border-amber-500"   },
              { k:"high",   active:"bg-red-500     text-white border-red-500"     },
            ].map(p => (
              <button
                key={p.k} type="button" onClick={() => setPriority(p.k)}
                className={`flex-1 py-2 rounded-xl text-xs font-bold capitalize border transition-all ${
                  priority === p.k ? p.active : "bg-slate-50 text-slate-600 border-slate-200"
                }`}
              >{p.k}</button>
            ))}
          </div>
        ...
        <button
          type="submit"
          className="w-full py-3 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg shadow-teal-100 transition-all"
        >
          <Ic d={IC.send} size={14} /> Submit Complaint
        </button>
      </form>
    </Modal>
  );
